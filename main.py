...
while True:
    try:
        ...
        WebDriverWait(driver, 10).until(presence_of_element_located((By.CSS_SELECTOR, 'a.group-toggle-link')))
...
WebDriverWait(driver, 20).until_not(presence_of_element_located((By.XPATH, '//*[contains(text(), "Opdaterer ...")]')))
...
while moreApplications:
    try:
        WebDriverWait(driver, 20).until(element_to_be_clickable((By.XPATH, '//*[contains(text(), "Vis flere ejendomme")]')))
        WebDriverWait(driver, 20).until_not(presence_of_element_located((By.XPATH, '//*[contains(text(), "Opdaterer ...")]')))
...
for url in APPLICATION_URLS:
    ...
    WebDriverWait(driver, 20).until(presence_of_element_located((By.CLASS_NAME, 'group-toggle-link')))
    ...
    for t in driver.find_elements(By.CLASS_NAME, 'group-toggle-link'):
        ...
        rooms = int(search("#collapse-(?P<r>\d)", t.get_attribute('href')).group('r'))
        ...
        areaAndprices = search("(?P<a1>\d*)-(?P<a2>\d*).*, (?P<p1>\d*)-(?P<p2>\d*) kr.", areapriceString)
        ...
        for c in tbody.find_elements(By.TAG_NAME, 'tr'):
            ...
            area = int(search(" (?P<a>\d*) m", cc[1].get_attribute('innerText')).group('a'))

            # Pessimistic estimation of price, according to highest price / biggest area
            estPrice = int((prices[1] / areas[1]) * area)
            if (area == areas[0]):
                estPrice = prices[0]
            ...
            try:
                ...
                rank = search("\w", rankString).group(0)
            except:
                rank = "Z?"
...
for a in sortedApps:
    ...
    sheet["B"+str(it)] = a.amount
    sheet["C"+str(it)] = a.name
    sheet["D"+str(it)] = a.rooms
    sheet["E"+str(it)] = a.grossArea
    sheet["F"+str(it)] = a.estPrice
